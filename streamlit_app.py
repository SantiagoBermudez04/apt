...
def obtener_tickers_sp500():
    ...
    # Intento 1: Usando storage_options nativo de Pandas
    try:
        tabla = pd.read_html(
            url,
...
def obtener_datos(tickers, factores, periodo):
    ...
    try:
        ...
        precios = precios.resample('W').last().ffill()
...
for i in range(num_activos):
    ...
    with col2:
        peso_default = 40 if i == 0 else 30
...
if analizar_btn or "analizado" in st.session_state:
    ...
    with st.spinner("Validando activos y calculando modelo..."):
        ...
        # ==========================================
        # C1. Regresión Múltiple para obtener las Betas controladas
        ...
        # C1.b Cálculo del R2 individual usando la correlación de Pearson al cuadrado
        r2_factores = {}
        for factor in factores_macro:
            correlacion = y_port.corr(retornos[factor])
            r2_factores[factor] = correlacion ** 2
            
        r2_petroleo = r2_factores['CL=F']
        r2_interes = r2_factores['^TNX']
        r2_crecimiento = r2_factores['^GSPC']

        # C2. Regresiones individuales para cada activo (para el gráfico)
        betas_individuales = []
        for ticker in tickers:
            ...
            betas_individuales.append({
                ...
                'Cartera de Mercado (S&P 500)': mod_ind.params['^GSPC']
            })
    ...
    st.markdown("#### Varianza Explicada (R² Individuales)")
    c4, c5, c6 = st.columns(3)
    c4.metric("R² Petróleo", f"{r2_petroleo:.2%}", help="Porcentaje de los movimientos del portafolio explicados exclusivamente por el Petróleo.")
    c5.metric("R² Tasa de Interés", f"{r2_interes:.2%}", help="Porcentaje de los movimientos del portafolio explicados exclusivamente por las Tasas de Interés.")
    c6.metric("R² Mercado", f"{r2_crecimiento:.2%}", help="Porcentaje de los movimientos del portafolio explicados exclusivamente por el S&P 500.")
    ...
    if beta_crecimiento > 1.5:
        msg = "⚠️ Alta exposición al factor cartera de mercado detectada (β_crecimiento > 1.5). Tu portafolio es altamente pro-cíclico."
        st.warning(msg)
        alertas_activas.append("Fuerte componente pro-cíclico (Dependencia del PBI/Mercado)")
    if beta_crecimiento < 0:
        msg = "⚠️ Tu cartera tiene una Beta de Cartera de Mercado negativa. Se comporta de manera contra-cíclica (defensiva)."
        st.info(msg)
    ...
    df_melt = df_betas.melt(id_vars='Ticker', value_vars=['Petróleo', 'Tasa de Interés', 'Cartera de Mercado (S&P 500)'],
                            var_name='Factor', value_name='Beta')
    
    port_data = pd.DataFrame({
        'Ticker': ['Portafolio']*3,
        'Factor': ['Petróleo', 'Tasa de Interés', 'Cartera de Mercado (S&P 500)'],
            ...
            if "ai_response" not in st.session_state or st.session_state.get("last_state_key") != (tickers + pesos + [api_key]):
                with st.spinner("Analizando tu portafolio con Google Gemini..."):
                    try:
                        ...
                        - Beta Petróleo (CL=F): {beta_petroleo:.2f} (Explica el {r2_petroleo:.2%} de la varianza)
                        - Beta Tasa de Interés (^TNX): {beta_interes:.2f} (Explica el {r2_interes:.2%} de la varianza)
                        - Beta Cartera de Mercado (^GSPC): {beta_crecimiento:.2f} (Explica el {r2_crecimiento:.2%} de la varianza)
                        ...
                        1. PERFIL DE RIESGO MULTIFACTORIAL
                           - Interpretación de cada Beta y su R² individual en términos concretos y comprensibles para el inversor minorista. Usa números concretos.
